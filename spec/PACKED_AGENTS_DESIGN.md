...
| **PA.2** | work-queue refill: worker signals done (EOG / `<<<DONE>>>`) → harness prefills the next backlog piece into the freed lane | **pool mechanism DONE (2026-06-11):** `--pool M` runs M items over N lanes, refill-on-done; **8 items/4 lanes = 34.2 tok/s (1.77×), 4 refills** — vs PA.1b's 11 tok/s straggler. Remaining: feed the pool from the boss's task list (boss queue → worker queue) + idle-lane stats. |
