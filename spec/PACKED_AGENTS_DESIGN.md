...
| **PA.4** | bidirectional coordination → became a **verify→repair loop** | **DONE (2026-06-15):** harness test-gen → run tests → **L1 worker amend** → **L2 boss arbiter** re-queues rework on `--repair-budget` exhaustion (the GUIDE/KILL/RESPEC authority, realized as queue ops). Fresh sessions; **full-triad rework** (goal/contract + blueprint + module + test + error); repair lanes think. `--coord-test` 10/10. See `PA4_COORDINATION_DESIGN.md`. |
...
| **PA.7** | eager scheduling (dissolve the stage barriers) | **DESIGNED (2026-06-15):** artifact-gated ready-queue — a free lane pulls the next ready item *across* stages instead of idling at a barrier; **reconcile becomes the first rework pass** (contract-diff → rework, same primitive as post-test repair). `--eager-test` planned. See `PA7_PIPELINING_DESIGN.md`. |
