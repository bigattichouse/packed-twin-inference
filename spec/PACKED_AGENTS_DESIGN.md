...
| **PA.3** | speculation stacking (MTP/lookup per stream, `n_seq_max=8`) | **DONE (2026-06-15, behind `--mtp`):** MTP second context + nextn head + checkpoint-seq rollback, `--mtp-test` 8/8. **MEASURED net-slower on packed** — doubling `n_seq_max` taxes every lane's SSM state more than spec-dec saves; kept opt-in + as a diagnostic. The hoped ~2.4× does not hold on this hybrid card. See `PA3_MTP_DESIGN.md`. |
| **PA.4** | bidirectional coordination → became a **verify→repair loop** | **DONE (2026-06-15/16):** harness test-gen → run tests → **L1 worker amend** → **L2 boss arbiter** re-queues rework via a **work-order** (PLAN/PIECE — the format the boss reliably emits; old bespoke REWORK parse got 0 items). Fresh sessions get the **full triad + collaborator code** (goal/contract + blueprint + module + test + error + the modules it calls); repair lanes think; **escalate early** when L1 stalls; **designer dictates libraries** (contract pins deps/CommonJS, no jsdom); **untested modules re-queue test-gen**; **repair journal** (per-component `ATTEMPT:` history fed to the next worker + arbiter), **multi-round arbiter**, **RESPEC-on-stuck** (rewrite the living blueprint). `--coord-test` 14/14. See `PA4_COORDINATION_DESIGN.md`. |
...
| **PA.6** | staged design→build pipeline (parallelize the serial plan-think) | **DONE (2026-06-15):** triage → parallel design (blueprints) → reconcile (interface contract) → parallel implement → test-gen → verify → repair, on `--tools --no-stream`. Reconcile lifted verify **1/3→2/3** (fixed canvas-vs-ctx drift). GPU end-to-end validating. See `PA6_PIPELINE_DESIGN.md`. |
| **PA.7** | eager scheduling (dissolve the stage barriers) | **CORE BUILT (2026-06-16); integration pending:** artifact-gated ready-queue — a free lane pulls the next ready item *across* stages instead of idling at a barrier; **reconcile = the first rework pass**; **active retrieval** (`read_file`/`abandon`, NOT_FOUND → re-queue gated on the file). Pure scheduler + makespan sim (`--eager-test` 8/8: **eager 18% < barriers**) + active-tool primitives (`--gather-test` T5–T8) in; `run_pool` mid-stream loop + ready-queue orchestration next. See `PA7_PIPELINING_DESIGN.md`. |
